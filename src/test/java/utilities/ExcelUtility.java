...
public class ExcelUtility {
    ...
    public static void printAllData(){
        ...
        //outer loop, that loops through rows
        try {
            for (int i = 0; i < totalNoOfRows; i++) {
                int totalNoOfCols = workSheet.getRow(i).getPhysicalNumberOfCells();
                for (int j = 0; j < totalNoOfCols; j++) {
                    System.out.print(workSheet.getRow(i).getCell(j).getStringCellValue()+"\t");
                }
                System.out.println();
            }
        }catch (Exception e){
